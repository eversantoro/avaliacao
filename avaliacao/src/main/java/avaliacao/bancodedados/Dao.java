...
/**
 * Classe Generica para a manipulacao de objetos
...
public abstract class Dao<T extends Serializable> implements Serializable {
	...
	@SuppressWarnings("unchecked")
	public List<T> getTodos(int rownum) throws Excecoes {
	
		EntityManager em = Conexao.getConexaoEM();
		try {
			Query listQuery = em.createQuery("select object(c) from "+ nomeEntidade +" as c where rownum <= " + rownum );
			return listQuery.getResultList();
		} catch(Exception e) {
			throw new Excecoes(e,"pesquisarErro");
		} finally {
	...
	/**
	 * Metodo responsaves por realizar a pesquisa atraves de um objeto populado
	 * @param entidade - objeto que contenha os atributos para a pesquisa
	 * @param classeInterna - array contendo as classes que deverao ser utilizadas no Left Join
	 * @param ordem - nome dos atributos para serem utilizados no order by da query
	 * @return lista do objeto com o retorno da pesquisa
	 * @throws Excecoes
	 */
	...
	/**
	 * Metodo responsavel por controlar a recursividade dos atributos de entidades do objeto
	 * @param entidade - objeto que contenha os atributos para a pesquisa
	...
	/**
	 * Metodo responsavel por formatar o nome de entidade para ser utilizado na query de busca
	 ...
	 * @return String contendo a instrucao de left join para a query
	 */
	@SuppressWarnings("rawtypes")
